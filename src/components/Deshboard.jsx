...
const AppointmentRow = ({ appointment_index, patient_name, time_slot, kalra_id, onUpdateClick, pay_id, refund_id, data }) => {
    const navigate = useNavigate();
    
    // Check if ID already exists (truthy check)
    const hasId = !!kalra_id && kalra_id.trim() !== "";

    // Determine the status label and styling dynamically
    ...
    return (
        <div 
            onClick={() => navigate(`/appointment/${encodeURIComponent(JSON.stringify(data))}`)} 
            className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-4 px-4 bg-white hover:bg-gray-50 border-b border-gray-100 last:border-b-0 transition duration-150 cursor-pointer"
        >
            <div className="flex-1 min-w-0 mb-3 sm:mb-0">
                ...
                <div className="flex items-center mt-1 space-x-2">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${status.classes}`}>
                        {status.label}
                    </span>
            ...
            {Cookies.get('user') !== '67ee5e1bde4cb48c515073ee' && (
