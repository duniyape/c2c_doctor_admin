...
export default function DoctorLedgerPage({fromDate,toDate , sendDataToParent}) {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = `https://api.care2connect.in/v1/doctor/${doctorId}?from=${fromDate}&to=${moment(toDate).add(1, 'days').format("YYYY-MM-DD")}`;
 ...
 const fetchLedger = async () => {
  const doctorId = Cookies.get("user");

  // Dates select nahi hain to API call mat karo
  if (!fromDate || !toDate) {
    return;
  }
  ...
  const url = `https://api.care2connect.in/v1/doctor/${doctorId}?from=${fromDate}&to=${moment(toDate)
    .add(1, "days")
    .format("YYYY-MM-DD")}`;

  try {
    console.log("Doctor Ledger API Call:", url);
    ...
    Swal.fire(
      "Connection Failed",
      "Could not load doctor ledger. Try later.",
      "error"
    );
  } finally {
    setLoading(false);
...
useEffect(() => {
  ...
  console.log("Date changed:", fromDate, toDate);
  ...
  return (
    <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e5e7eb", boxShadow: "0px 4px 12px rgba(0,0,0,0.05)" }}>
      ...
      >
        ...
        {/* <div>
          <label style={{ display: "block", fontSize: "14px", fontWeight: "600", color: "#374151", marginBottom: "8px" }}>From Date</label>
          <input
            type="date"
            name="from"
            value={formData.from}
